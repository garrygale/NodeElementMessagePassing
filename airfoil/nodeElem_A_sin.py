# -*- coding: utf-8 -*-
"""
Last version @ Dec. 26, 2022, 19:49 PT
...
class nodeBlock(nn.Module):
    ...
    def __init__(self,nin,nElementNode,nElement,nout,hidden_dim):
        super().__init__()
        self.nodeMLP = siren(nin+nElementNode+nElement,nout,hidden_dim)
...
# torch.save(network.state_dict(),'nodeElem_A_sin_Airfoil_NoNoise.pt')

network.cpu()
network.load_state_dict(torch.load('nodeElem_A_sin_Airfoil_NoNoise.pt'))
network.to(device)
network.eval()
...
for j in range(testInU.shape[0]):
    ...
    for ii in range(nStepRollout):
        ...
        #         torch.save(useNodesBack,'airfoil_nodeElemAlt_sin_'+str(ii+1)+'_'+str(testRe[j].item())+'.pt')

print('NN time per step ',NNtime/nStepRollout/testInU.shape[0])
print('overhead per step ',otherTime/nStepRollout/testInU.shape[0])

# NN time per step  0.017313870182743777
# overhead per step  0.0001376563443077935
...
# torch.save(r2Save,'airfoil_nodeElemAlt_sin.pt')
