...
class nodeBlock(nn.Module):
    ...
    def __init__(self,nin,nElement,nout,hidden_dim):
        super().__init__()
        self.nodeMLP = MLPnorm(nin+nElement,nout,hidden_dim)
...
# torch.save(network.state_dict(),'nodeElem_B_Airfoil_NoNoise.pt')

network.cpu()
network.load_state_dict(torch.load('nodeElem_B_Airfoil_NoNoise.pt'))
network.to(device)
network.eval()
...
for j in range(testInU.shape[0]):
    ...
    for ii in range(nStepRollout):
        ...
        #         torch.save(useNodesBack,'airfoil_nodeElem_gelu_'+str(ii+1)+'_'+str(testRe[j].item())+'.pt')

print('NN time per step ',NNtime/nStepRollout/testInU.shape[0])
print('overhead per step ',otherTime/nStepRollout/testInU.shape[0])

# NN time per step  0.014752654596611304
# overhead per step  9.203494919670952e-05
...
# torch.save(r2Save,'airfoil_nodeElem_gelu.pt')
